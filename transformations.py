...
def normalize_01_clip99(inp: np.ndarray):
    """Squash image input to the value range [0, 1] after clipping 1-99
    DIMS C, L, W, H
    does it separately for each channel"""
    if inp.shape[0] > 1:
        maxs = inp.max(axis=(1,2,3))
        mins = inp.min(axis=(1,2,3))
        ranges = maxs - mins
        cuts = (0.01 * ranges)
        clip0 = mins + cuts
        clip1 = maxs - cuts
        inp0 = inp[0].clip(clip0[0], clip1[0])
        inp0 = (inp0 - inp0.min() )/ inp0.ptp()
        inp1 = inp[1].clip(clip0[1], clip1[1])
        inp1 = (inp1 - inp1.min()) / inp1.ptp()
        inp_out = np.stack([inp0, inp1]).astype(np.float32)
    else:
        range = np.max(inp) - np.min(inp)
        cut = 0.01 * range
        clip0 = np.min(inp) + cut
        clip1 = np.max(inp) - cut
        inp = np.clip(inp, clip0, clip1)
        inp_out = (inp - np.min(inp)) / np.ptp(inp)
    return inp_out


def normalize_dual(inp: np.ndarray):
    """Squash image input to the value range [0, 1] after clipping 1-99"""
    range = np.max(inp) - np.min(inp)
    cut = 0.01 * range
    clip0 = np.min(inp) + cut
